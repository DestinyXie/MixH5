...
var defaultConfig = {
    ...
    logApi: 'http://139.224.2.8:6666/api/sharelog', // 统计日志接口
...
function setShare(shareConfig) {
    function _doSetSahre() {
        ...
        if (logConfig && logConfig.openid) {
            var queryParams = util.getUrlQuery();
            logConfig.fromid = queryParams.fromid || 'root';
            logConfig.device = util.getDeviceName(); // 设备
            ...
            if (shareLink.lastIndexOf('?') < 0) {
                shareLink = shareLink + '?fromid=' + logConfig.fromid;
            }
            else {
                shareLink = shareLink + '&fromid=' + logConfig.fromid;
            }
        ...
        wx.onMenuShareTimeline({
            ...
            link: shareLink + '&spreadtype=timeline', // 分享链接
        ...
        wx.onMenuShareAppMessage({
            title: shareConfig.title, // 分享标题
            link: shareLink + '&spreadtype=message', // 分享链接
            imgUrl: shareConfig.img, // 分享图标
